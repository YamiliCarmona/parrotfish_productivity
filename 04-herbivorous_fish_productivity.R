...
merged_data <- readRDS("data/fishdata_sstmean_maxsize_by_year_herbivoros.RDS") |> 
  select(-MaxSizeTL)

tabla <- read_csv("data/spp_parametros_2024-05-22.csv")|> 
  # select(-MaxSizeTL) |>
...
merge_database <- merge(merged_data, tabla, by = c("Family", "Species", "A_ord", "B_pen"), all.x = TRUE)  |> 
  rename( a = "A_ord", #Diet = TrophicGroup
         b = "B_pen") 
  


aver <-merge_database |> 
  # filter(Year == 2022, Region == "La Paz") |> 
  filter(is.na(sstmean)) |>
  distinct(Year, Region, Reef, sstmean)
...
merge_database <- merge_database |> 
  ...
  #   Reef = str_trim(Reef),
  #   sstmean = case_when(
  #     Reef == "CORONADO_PUNTA_LOBOS" & Year == 2016 ~ 25.6,
  #     Reef == "PUNTA_SAN_RAFAELITO" & Year == 2022 ~ 24.8,
  #     TRUE ~ sstmean
  #   )
  # ) |> 
  filter(!Size == 0) |>
...
#
# saveRDS(datagr_prod, "data/fish_datagr_prod-by-species-herbivoros2.RDS")


# prod by reef---------------

# datagr_prod <- readRDS("data/fish_datagr_prod-by-species-herbivoros2.RDS")
...
# saveRDS(data_prod_brut, "data/fishdata_prod-by-reef-herbivoros.RDS")
...
# ggsave("herb/gestion_herbivoros.png", width = 12, height = 8, dpi=1000)
